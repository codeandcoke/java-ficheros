...
public class Ejemplo {
	...
	private void nuevoJugador() {
		...
		if ((txtNombreJugador.equals("")) || (txtApellidos.getText().equals(""))) {
		...
		try {
			...
			if (txtFechaNacimiento.getText().equals("")) {
				...
				jugador.setFechaNacimiento(formatFecha.parse(txtFechaNacimiento.getText()));
			}
			jugador.setDireccion(txtDireccion.getText());
			jugador.setTelefono(txtTelefono.getText());
			jugador.setEquipo(cbEquipo.getEquipo());
			
			jugadores.add(jugador);
			Ficheros.escribirObjeto(jugadores, Constantes.FICHERO_JUGADORES);
			
			panelEstado.setMensaje("Jugador dado de alta correctamente");
	...
	private void nuevoEquipo() {
		...
		txtPatrocinador.setEditable(true);
		txtPatrocinador.setText("");
		txtColor1.setEditable(true);
		txtColor1.setText("");
		txtColor2.setEditable(true);
		txtColor2.setText("");
		txtCategoria.setEditable(true);
		txtCategoria.setText("");
		
		btNuevoEquipo.setEnabled(false);
		btGuardarEquipo.setEnabled(true);
		btCancelarEquipo.setEnabled(true);
				
		tablaEquipos.setEnabled(false);
		
		accion = Accion.NUEVO;
		panelEstado.setColorEstado(Color.GREEN);
		panelEstado.setEstado("NUEVO");
	}
	...
	private void guardarEquipo() {
		...
		case NUEVO:
			...
			equipo.setPatrocinador(txtPatrocinador.getText());
			equipo.setColor1(txtColor1.getText());
			equipo.setColor2(txtColor2.getText());
			equipo.setCategoria(txtCategoria.getText());
			...
			try {
				...
				panelEstado.setMensaje("El equipo se ha dado de alta correctamente");
				limpiarCamposEquipo();
	...
	private void apagarControlesEquipo() {
		...
		btNuevoEquipo.setEnabled(true);
		btGuardarEquipo.setEnabled(false);
		btCancelarEquipo.setEnabled(false);
	}
	...
	private void cargaEquipoDeTabla() {
		...
		txtPatrocinador.setText(equipo.getPatrocinador());
		txtColor1.setText(equipo.getColor1());
		txtColor2.setText(equipo.getColor2());
		txtCategoria.setText(equipo.getCategoria());
	}
	...
	private void seleccionarEquipo() {
		...
		btModificarEquipo.setEnabled(true);
		btEliminarEquipo.setEnabled(true);
	}
	...
	private void modificarEquipo() {
		...
		btGuardarEquipo.setEnabled(true);
		btCancelarEquipo.setEnabled(true);
		btEliminarEquipo.setEnabled(false);
		...
		panelEstado.setColorEstado(Color.GREEN);
		panelEstado.setEstado("MODIFICAR");
	}
	...
	private void eliminarEquipo() {
		...
		nombreEquipo = tablaEquipos.getNombreEquipoSeleccionado();
		...
		try {
			equipos.remove(tablaEquipos.getEquipoSeleccionado());
			Ficheros.escribirObjeto(equipos, Constantes.FICHERO_EQUIPOS);
			tablaEquipos.listar();
			cbEquipo.listar();
			
			panelEstado.setMensaje("El equipo " + nombreEquipo + " se ha eliminado con éxito");
		} catch (IOException ioe) {
	...
	private void aplicarFiltroEquipo() {
		...
		if (txtFiltro.getText().length() <= 1) {
			return;
		}
		
		tablaEquipos.listar(txtFiltro.getText());
	}
	...
	private void initialize() {
		...
		frmPcFutbolDam.setTitle("PC Futbol DAM");
		frmPcFutbolDam.setResizable(false);
